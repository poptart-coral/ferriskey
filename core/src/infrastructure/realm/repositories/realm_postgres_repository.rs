...
impl RealmRepository for PostgresRealmRepository {
    ...
    async fn update_realm_setting(
        ...
        if let Some(magic_link_enabled) = magic_link_enabled {
            realm_setting.magic_link_enabled = Set(magic_link_enabled);
        }

        if let Some(magic_link_ttl_minutes) = magic_link_ttl_minutes {
            realm_setting.magic_link_ttl_minutes = Set(magic_link_ttl_minutes as i32);
        }
